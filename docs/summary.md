...
**问题**：服务器重启后仍然重复抓取 domain-trends，原因有两个：

1. **`loadFromDisk()`**：只加载 `domain-trends:web3`，不加载轮换模式的 `domain-trends:web3:group0`
2. **`cleanupOldData()`**：只保留 00, 08, 16 时间点，会删除轮换模式的 02, 04, 06 等数据

**修复**：

1. **`loadFromDisk()`**：改为扫描缓存目录所有 `.hourly.json` 文件，自动恢复所有缓存
2. **`cleanupOldData()`**：轮换模式保留所有 2 小时窗口点（00, 02, 04, ...）

---

### 3.31 修复轮换模式缓存读取窗口不匹配 (v2.9.11)

**问题**：`skillCache.get()` 使用当前小时作为 hourKey，但轮换模式数据存储在 2 小时窗口起点。

**修复**：`get()` 根据 skillId 类型使用正确的时间窗口。

---

### 3.30 domain-trends 轮换模式时间轴显示修复 (v2.9.10)

**问题**：domain-trends 轮换模式每 2 小时执行一次，但时间轴显示仍然是 8 小时间隔（0:00, 8:00, 16:00）。

**修改**：更新 `skillCache.js` 支持 2 小时间隔：

1. **`getAvailableHours()`**：检测轮换模式缓存 key（包含 `:group`），使用 2 小时间隔
2. **`getDomainTrendsRotationHours()`**：新增函数，返回过去 12 个 2 小时时间点
3. **`set()`**：轮换模式存储到 2 小时窗口起点（00, 02, 04...）

**时间轴显示对比**：

| 模式 | 时间点 |
|------|--------|
| 轮换模式 | 0:00, 2:00, 4:00, 6:00, 8:00, 10:00, 12:00, 14:00, 16:00, 18:00, 20:00, 22:00 |
| 传统模式 | 0:00, 8:00, 16:00 |
